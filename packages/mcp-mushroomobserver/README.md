...
- For "look up a name" use `search_fungal_names`. It returns the scientific name + author + classification + a link to the MO name page.
...
- All tools return the same `Result<T>` envelope: success → `{ ok: true, data }`; error → `isError: true` with `{ ok: false, error: { source, message, statusCode? } }`.
