...
- For "when does X bloom in region Y?" use `get_site_level_data` with state filter — gives you mean first/last yes dates per phenophase.
- Avoid calling `get_observations` without species filtering — single station-year results can be 80+ MB.
- All tools return the same `Result<T>`-style envelope: success → `{ ok: true, data }`; error → `isError: true` with structured `{ ok: false, error: { source, message, statusCode? } }`.
