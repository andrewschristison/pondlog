...
and constellation visibility, curated rather than raw catalogs.
...
| `getDarkSkyWindow` | `{ coords, date? }` | astronomical dark window (sun ≤ -18°), 1-to-5 quality score, moon altitude/illumination at midpoint |
...
- `scoreDarkSky({ hasAstronomicalDark, moonIlluminationFraction, moonAltitudeDeg })`: the 1-to-5 quality function used by `getDarkSkyWindow`.
...
astronomical dark** following the reference time (when stargazers are
actually out), falling back to the midpoint between sunset and
